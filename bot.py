...
async def auto_image_dropper():
    ...
    for channel in discord_client.get_all_channels():
        if channel.name in image_channels and isinstance(channel, discord.TextChannel):
            try:
                # We add a random number to the prompt so Pollinations generates a totally new image every time
                ...
                # ⏳ Wait 5 seconds before dropping the next image so we don't spam the API
