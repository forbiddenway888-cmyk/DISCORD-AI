...
async def on_message(message):
    ...
    try:
        ...
        if "[DRAW]" in bot_reply_clean:
            ...
            # --- THE ACTUAL VIDEO ENGINE (POLLINATIONS AI) ---
            async with message.channel.typing():
                # Grab the new key from Render
                polli_key = os.getenv("POLLI_KEY")
                
                # We have to URL-encode the text so it works in a web link
                import urllib.parse
                safe_prompt = urllib.parse.quote(video_prompt)
                
                # The brand new video endpoint
                api_url = f"https://gen.pollinations.ai/video/{safe_prompt}"
                headers = {"Authorization": f"Bearer {polli_key}"}
                
                try:
                    timeout = aiohttp.ClientTimeout(total=210) 
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        # Notice this is a GET request now, not a POST
                        async with session.get(api_url, headers=headers) as resp:
                            if resp.status == 200:
                                ...
                                await message.reply(f"Bro, the API rejected it. Error: `{resp.status} - {error_text}` (Diamond refunded 💎)")
                                user_diamonds[user_id]["diamonds"] += 1 
                                
                except asyncio.TimeoutError:
                    await message.reply("Bro, the AI took too long to render and timed out. (Diamond refunded 💎)")
                    user_diamonds[user_id]["diamonds"] += 1
                except Exception as e:
                    print(f"Network Crash: {e}")
                    await message.reply(f"Bro, the connection dropped. Raw Error: `{str(e)}` (Diamond refunded 💎)")
                    user_diamonds[user_id]["diamonds"] += 1
