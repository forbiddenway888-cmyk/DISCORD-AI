...
async def on_message(message):
    ...
    # ==========================================
        ...
        if bot_reply.startswith("[DRAW]"):
            ...
            # Groq decided to generate a video!
            video_prompt = bot_reply.replace("[VIDEO]", "").strip()
            
            async with message.channel.typing():
                safe_prompt = urllib.parse.quote(video_prompt)
                
                # The secret free Pollinations Video API endpoint
                video_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?enhance=true&model=flux&nologo=true&feed=true"
                
                # Because it's a video, Discord embed won't play it from a URL.
                # We have to download it temporarily and upload it as a real file.
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(video_url) as resp:
                            if resp.status == 200:
                                ...
                                # Delete the temporary file so Render's hard drive doesn't get full
                                os.remove(temp_filename)
                            else:
                                await message.reply("Bro, the video engine is resting right now. Try again in a minute.")
                except Exception as e:
