...
client_chat = discord_client  # Keeps your existing bot variable intact
client_draw = discord.Client(intents=intents)

# Define Pollinations Image URL Base
import urllib.parse
...
async def on_message(message):
    ...
    # Ignore messages unless this specific second bot is tagged
    if not client_draw.user.mentioned_in(message): 
        return

    try:
        async with message.channel.typing():
            # Strip the bot's tag out to extract just the prompt text
            prompt = message.content.replace(f'<@{client_draw.user.id}>', '').strip()
            
            if not prompt:
                await message.reply("Bro, tell me what to draw! Tag me and write your prompt.")
                return
            
            # Encode and generate the image via keyless Pollinations API
            safe_prompt = urllib.parse.quote(prompt)
            image_url = f"https://image.pollinations.ai/prompt/{safe_prompt}"
            
            # Create a professional look with a Discord embed
