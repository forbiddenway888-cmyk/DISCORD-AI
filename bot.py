...
async def auto_image_dropper():
    # Defines the exact prompts for each specific channel
    image_channels = {
        "𓆩︱pfps︱𓆪": "cool edgy anime profile picture avatar, dark aesthetic, high quality, luxury",
        "𓆩︱banners︱𓆪": "wide landscape banner background, aesthetic dark luxury, cyberpunk city, highly detailed",
    ...
    for channel in discord_client.get_all_channels():
        if channel.name in image_channels and isinstance(channel, discord.TextChannel):
            try:
                ...
                unique_prompt = f"{image_channels[channel.name]}, variation {random_seed}"
                safe_prompt = urllib.parse.quote(unique_prompt)
                
                # Banners need to be wide (1024x400), PFPs and Icons are square (512x512)
                if "banners" in channel.name:
                    image_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?width=1024&height=400&nologo=true"
                else:
                    image_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?width=512&height=512&nologo=true"
                
                # ⬇️ THE FIX: Download the image first so Discord doesn't timeout!
                async with aiohttp.ClientSession() as session:
                    async with session.get(image_url) as resp:
                        if resp.status == 200:
                            ...
                            # Attach it to the embed natively
                            file = discord.File(temp_filename, filename="drop.png")
                            embed = discord.Embed(title="🔥 Fresh AI Drop", description="Steal this for your profile.", color=discord.Color.dark_gray())
                            embed.set_image(url="attachment://drop.png")
                            embed.set_footer(text="Auto-generated by FORBID AI")
                            
                            await channel.send(embed=embed, file=file)
                            
                            # Clean up the file from your server after sending
                            os.remove(temp_filename)
                
                # Wait 5 seconds before moving to the next channel
