            ...
            async with message.channel.typing():
                try:
                    def hijack_web_demo():
                        ...
                        # We are passing exactly 10 settings, which is what their server expects
                        result = client.predict(
                            video_prompt,            # 1. prompt
                            "blurry, bad quality",   # 2. negative_prompt
                            None,                    # 3. input_image (None)
                            None,                    # 4. input_video (None)
                            512,                     # 5. height_ui
                            704,                     # 6. width_ui
                            "text-to-video",         # 7. mode
                            2.0,                     # 8. duration_ui (seconds)
                            9,                       # 9. ui_frames_to_use
                            42,                      # 10. seed_ui
                            fn_index=0               # The magic bypass
                        )
                        return result[0] 

                    video_path = await asyncio.to_thread(hijack_web_demo)
                    
                    file = discord.File(video_path, filename="forbid_200IQ_video.mp4")
