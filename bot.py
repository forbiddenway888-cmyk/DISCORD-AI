...
async def on_message(message):
    ...
    # ==========================================
        # THE AI ROUTER (INTERCEPTING IMAGE & VIDEO)
        # ==========================================
        if bot_reply.startswith("[DRAW]"):
            # Groq decided to draw! Isolate the prompt text
            image_prompt = bot_reply.replace("[DRAW]", "").strip()
        ...
        elif bot_reply.startswith("[VIDEO]"):
            # Groq decided to generate a video! Isolate the action prompt
            video_prompt = bot_reply.replace("[VIDEO]", "").strip()
            
            async with message.channel.typing():
                safe_prompt = urllib.parse.quote(video_prompt)
                
                # FREE TEXT-TO-VIDEO HACK:
                # Pollinations has a hidden parameter feeding into motion frameworks that outputs high-speed looping animations/GIFs entirely for free without API keys.
                video_url = f"https://image.pollinations.ai/prompt/{safe_prompt}?enhance=true&model=flux&nologo=true&feed=true"
                
                display_title = f"🎥 {video_prompt}"
                if len(display_title) > 256:
                    display_title = display_title[:253] + "..."
                    
                embed = discord.Embed(title=display_title, color=discord.Color.blue())
                embed.set_image(url=video_url)
                embed.set_footer(text="Generated by FORB1D🔥 via Video Engine")
                await message.reply(embed=embed)
