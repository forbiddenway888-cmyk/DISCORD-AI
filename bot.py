    ...
    try:
        ...
        if "[DRAW]" in bot_reply_clean:
            ...
            async with message.channel.typing():
                ...
                try:
                    # Give it a massive 3.5 minute timeout because videos take forever to render
                    ...
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        async with session.post(api_url, headers=headers, json=payload) as resp:
                            if resp.status == 200:
                                ...
                                await message.reply("Bro, the free video engine is currently booting up its GPUs. Give it about 60 seconds and try again! (Diamond refunded 💎)")
                                user_diamonds[user_id]["diamonds"] += 1 
                            else:
                                error_text = await resp.text()
                                await message.reply(f"Bro, Hugging Face rejected the request. Error: `{resp.status} - {error_text}` (Diamond refunded 💎)")
                                user_diamonds[user_id]["diamonds"] += 1 
                                
                except asyncio.TimeoutError:
                    await message.reply("Bro, the video AI took too long to think and timed out. The servers are probably slammed. (Diamond refunded 💎)")
                    user_diamonds[user_id]["diamonds"] += 1
                except Exception as e:
                    print(f"Network Crash: {e}")
                    await message.reply(f"Bro, the connection to the video server dropped. Raw Error: `{str(e)}` (Diamond refunded 💎)")
                    user_diamonds[user_id]["diamonds"] += 1
